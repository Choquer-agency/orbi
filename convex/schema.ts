...
export default defineSchema({
  ...
  mailAccounts: defineTable({
    ...
    // One-time recipient-contact backfill state. Set once the chunked scan
    // finishes so the next deploy doesn't re-trigger it. Cleared by manual
    // "Rebuild contacts" action to allow a re-run.
    ...
    // Per-account color (CSS hex like "#FF7E16"). Renders as the ring around
    // the avatar in the thread list so the user can tell at a glance which
    // mailbox each thread came from. User-editable in Settings → Accounts;
    // when unset we fall back to a deterministic palette index on the client.
    color: v.optional(v.string()),
    // Send-as / inbox aliases the user owns on this provider account. Pulled
    // from Gmail's users.settings.sendAs (and the Microsoft equivalent).
    // Treated as "self" for reply-all recipient filtering so the user never
    // ends up in the To row when replying to a thread delivered to one of
    // these aliases. All entries are stored lowercase.
  ...
  emails: defineTable({
    ...
    // every other email in the mailbox (the Many-Bytes-Read 16MB error).
    .index("by_account_isDraft_receivedAt", [
      "accountId",
      "isDraft",
      "receivedAt",
    ])
  ...
  emailClassifications: defineTable({
    emailId: v.id("emails"),
    category: v.string(),
    // The trio below used to be required but isn't displayed anywhere any
    // more — kept as optional so we can drop them from new writes (saves
    // ~40% per row) without breaking older rows that still carry them.
    // A one-shot purge wipes them from existing rows; once that's done
    // they'll all be undefined.
