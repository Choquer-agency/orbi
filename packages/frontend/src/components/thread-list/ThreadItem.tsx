...
function ThreadItemImpl({ thread, isSelected, isMultiSelected, onSelect, onShiftClick, onCtrlClick, accountColor, onPrefetch }: ThreadItemProps) {
  ...
  // AI-driven spam classification was removed (see classifier.ts) — the spam
  // pill only fires when the upstream provider marked the thread as spam.
  ...
  const rowContent = (
    <div
      ...
      className={cn(
        'group relative flex w-full min-w-0 cursor-pointer items-center gap-3 overflow-hidden pl-3 pr-5 text-left transition-colors focus:outline-none focus-visible:outline-none',
    ...
    >
      ...
      {/* Concentric avatar: outer disc filled with the account color (2px stroke
          visible), a 2px cream/surface ring around the inner avatar, then the
          24px avatar. The inner avatar prefers the sender's company favicon and
